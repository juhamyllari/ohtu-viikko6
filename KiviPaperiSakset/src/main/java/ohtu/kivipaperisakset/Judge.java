package ohtu.kivipaperisakset;

/**
 * Judge pitää kirjaa ensimmäisen ja toisen pelaajan pisteistä sekä tasapelien
 * määrästä.
 */
public class Judge {

    private int ekanPisteet;
    private int tokanPisteet;
    private int tasapelit;

    public Judge() {
        this.ekanPisteet = 0;
        this.tokanPisteet = 0;
        this.tasapelit = 0;
    }

    public void judgeRound(String ekanSiirto, String tokanSiirto) {
        if (tasapeli(ekanSiirto, tokanSiirto)) {
            tasapelit++;
        } else if (ekaVoittaa(ekanSiirto, tokanSiirto)) {
            ekanPisteet++;
        } else {
            tokanPisteet++;
        }
    }

    /**
     * Sisäinen metodi, jolla tarkastetaan tuliko tasapeli.
     *
     * @return tasapeli
     */
    private static boolean tasapeli(String eka, String toka) {
        return eka.equals(toka);
    }

    /**
     * Sisäinen metodi joka tarkastaa voittaako eka pelaaja tokan.
     *
     * @param pelaajan 1 valinta
     * @param pelaajan 2 valinta
     * @return pelaaja 1 voittaa
     */
    private static boolean ekaVoittaa(String eka, String toka) {
        if ("k".equals(eka) && "s".equals(toka)) {
            return true;
        } else if ("s".equals(eka) && "p".equals(toka)) {
            return true;
        } else if ("p".equals(eka) && "k".equals(toka)) {
            return true;
        }
        return false;
    }

    @Override
    public String toString() {
        return "Pelitilanne: " + ekanPisteet + " - " + tokanPisteet + "\n"
                + "Tasapelit: " + tasapelit;
    }
}
